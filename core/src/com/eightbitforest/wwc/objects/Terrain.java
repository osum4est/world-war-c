...
import com.eightbitforest.wwc.objects.missiles.BaseMissile;
...
public class Terrain extends GameObjectDynamic {
    ...
    private boolean regenBody = false;

    public Terrain(int id, String image) {
        super(id, image);
    ...
    public void onCollideEnter(GameObjectDynamic other, Contact contact) {
        if (other instanceof BaseMissile) {
            if (!((BaseMissile) other).exploded) {
                explodeAtX((int) (other.body.getPosition().x * Globals.PPM), ((BaseMissile) other).radius);
                ((BaseMissile) other).explode();
            }
        }
    }
